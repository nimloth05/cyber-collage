...
export class SandGrain extends Agent {
  ...
  step() {
    // super.step();
    if (this.empty(-1, 0)) { this.move(-1, 0); } else if (this.empty(-1, -1)) { this.move(-1, -1); } else if (this.empty(-1, 1)) { this.move(-1, 1); }
  }
}
...
app.gallery.ifShapesReadyThen(shapes => {
  ...
  for (let i = 0; i < 0; i++) { app.agentCube.pushAgent(new ChickenAgent("Dog"), 0, 0); }

  for (let i = 0; i < 0; i++) { app.agentCube.pushAgent(new ChickenAgent("Rabbit"), 0, 0); }
