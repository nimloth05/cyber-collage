...
export class Agent {
    constructor(shapeName = "defaultShape", row = 0, column = 0, layer = 0) {
        ...
        this.hoverBox = new BoxHelper(this.shape, hoverBoxColor);
        this.hoverBox.agent = this;
        this.selectionBox = new BoxHelper(this.shape, selectionBoxColor);
    ...
    agentRelative(deltaRow, deltaColumn, deltaLayer = 0) {
        ...
        const agents = app.agentCube.grid[layer][row][column];
        const agent = agents[agents.length - 1];
        if (agent == null) { return null; } else { return agent; }
    }
    ...
    moveRandom() {
        ...
        if (this.isValidCoordinate(this.row + deltaRow, this.column + deltaColumn)) { this.move(deltaRow, deltaColumn); }
    }
